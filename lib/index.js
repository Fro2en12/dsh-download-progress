...
//   工作区文件增长监控（黑箱下载兜底）
//
// 仅依赖 cordis 注入服务（webServer），其余服务按需 ctx.get：
// fs、subprocess、sandboxPolicy、workspaceRegistry、tools。
// 注意：静态 host 组合没有 cordis timer 服务（动态沙箱专属），
// 定时器使用 Node 原生 setInterval 并随插件 Fiber 卸载清理。
...
export function apply(ctx) {
  ...
  ctx.effect(() => () => {
    ...
    timers.clear()
  }, 'dsh-download-progress: clear timers')
  ...
  // curl 解析：原版 subprocess.resolveExecutable 只按 PATH 搜裸命令名；
  // 本机 web 进程的 PATH 未必含 System32，故先走 PATH（通用），
  // 失败后再按平台回退到已知绝对路径。
  async function resolveCurl() {
    const candidates = process.platform === 'win32'
      ? ['curl', 'C:\\Windows\\System32\\curl.exe', 'C:\\Windows\\SysWOW64\\curl.exe']
  ...
  async function startDownload(args) {
    ...
    let exe
    try {
      exe = await resolveCurl()
    } catch (e) {
      return { ok: false, error: '未找到 curl 可执行文件（已尝试 PATH 与系统目录；Windows 10+ 自带 curl.exe）' }
    }
    const id = 'dl-' + (++seq)
    const t = makeTransfer({
